...
# Everything for an instance lives under instances/$(PROJECT_NAME)/ - the env file
# (instances/$(PROJECT_NAME)/.env) and the dhis2/postgresql config are generated by
# scripts/generate-env.sh and scripts/generate-instance-config.sh.
...
list-instances:
	@dirs=$$(ls -d instances/*/ 2>/dev/null); \
	if [ -z "$$dirs" ]; then \
...
# Requires: PROJECT_NAME to be set, ensure-networks and start-traefik and
#           start-monitoring to have been run first.
# Example: PROJECT_NAME=dev make start-instance
...
# Example: PROJECT_NAME=dev make delete-instance
delete-instance:
	@if [ -t 0 ]; then \
